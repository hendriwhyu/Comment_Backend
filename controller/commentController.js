...
exports.getComments = async (req, res) => {
  ...
  try {
    ...
    const comments = await prisma.comments.findMany({
      ...
      include: {
        User: {  // Sesuaikan nama relasi dengan model Anda
          select: {
            profile: {
              select: {
                name: true,
...
exports.createComment = async (req, res) => {
  ...
  const { postId } = req.params; // Pastikan rute endpoint menggunakan :postId

  try {
    // Ambil user ID dari request setelah otentikasi
    const userId = req.user.id;

    // Buat komentar baru pada post yang spesifik
    const newComment = await prisma.comments.create({
      data: {
        content,
        eventId: postId, // Asumsi bahwa postId adalah ID dari post yang ada
        userId: userId, // Menggunakan userId untuk menyimpan relasi ke pengguna
      },
    });
...
exports.updateComment = async (req, res) => {
  ...
  try {
    const comment = await prisma.comments.findUnique({
      ...
      include: {
        User: true, // Including the User to check the owner of the comment
      },
    });
